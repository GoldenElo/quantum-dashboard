        ...
        <LineChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
          ...
          <Legend
            formatter={(value: string) => {
              ...
              return <span style={{ color: '#5a6b82', fontSize: 12 }}>{lbl}</span>;
            }}
          />
          <Line type="monotone" dataKey="portfolio" stroke={portfolioColor} strokeWidth={2} dot={data.length < 30} activeDot={{ r: 4 }} />
          <Line type="monotone" dataKey="benchmark" stroke="#5a6b82" strokeWidth={1.5} strokeDasharray="5 3" dot={false} activeDot={{ r: 4 }} connectNulls={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
